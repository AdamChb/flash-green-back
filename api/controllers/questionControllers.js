...
const getKnownQuestionsByUserId = async (req, res) => {
  try {
    ...
    const questions = await Question.getKnownQuestionsByUserId(
      req.params.userId
    );
    res.status(200).json(questions);
  } catch (error) {
...
const getUnknownQuestionsByUserId = async (req, res) => {
  try {
    ...
    const questions = await Question.getUnknownQuestionsByUserId(
      req.params.userId
    );
    res.status(200).json(questions);
  } catch (error) {
...
const validateQuestion = async (req, res) => {
  try {
    ...
    const validatedQuestion = await Question.validateQuestion(
      questionId,
      req.user.id,
      isValid
    );
